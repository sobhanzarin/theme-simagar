	...
	class Puc_v4p6_Plugin_Ui {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('افزونه %s آخرین نسخه موجود در المنتور سایت بوده و نیازی به آپدیت ندارد!', 'the plugin title', 'plugin-update-checker'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('نسخه جدید برای محصول %s موجود است.', 'the plugin title', 'plugin-update-checker'), $title);
